...
export default function Options() {
  ...
  let defaulsLoopTime = 6
  let loopTimerLable = "List loading / assign time loop delay in seconds (defalt: " + {defaulsLoopTime} + " )"

  if (localStorage.getItem("options") === null || localStorage.getItem("options") === 'null') {
    console.log("options Var has been set to default")
    localStorage.setItem("options", JSON.stringify({stopBeforeShiftEnd: 10, unassigeTicketsFromOnLeaveAssignees: false , loopTimer: defaulsLoopTime}))
  }
  ...
  const passBackInfo = (options) => {
      ...
      temp.loopTimer = options.loopTimer
      setOptions(temp)
  }
  ...
  return (
    <Grid container spacing={1} >
        ...
        <Grid xs={11} >
            <TextField type="number" size="small" style={{minWidth: '400px'}} id="loopTimmer" label={loopTimerLable} variant="outlined" value={loopTimer}  onChange={handleUpdateLoopTimer} />
        </Grid>
        <Grid xs={10} >
          <FormControl>
